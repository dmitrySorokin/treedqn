...
class ReplayBuffer:
    def __init__(self, max_size=50000, start_size=10, batch_size=32):
        ...
        self.ret = np.zeros(max_size, dtype=float)
        ...
        self.treesize = np.zeros(max_size, dtype=float)

    def add_transition(self, obs, rew, act, nextobs, nextactset, done, treesize):
        self.insert_idx = self.insert_idx % self.max_size
        self.obs[self.insert_idx] = obs
        self.ret[self.insert_idx] = rew
        ...
        self.treesize[self.insert_idx] = treesize
