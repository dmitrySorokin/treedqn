...
class ReinforceAgent():
    def __init__(self, device) -> None:
        self.net = GNNPolicy().to(device)
        self.device = device

    def act(self, obs, act_set):
        with torch.no_grad():
            constraint_features, edge_indices, edge_features, variable_features = \
            torch.from_numpy(obs.row_features.astype(np.float32)).to(self.device), \
            ...
            logits = self.net(constraint_features, edge_indices, edge_features, variable_features)[act_set.astype(np.int32)]
            action_idx = torch.argmax(logits)
            action = act_set[action_idx.item()]
        return action
